...
void twoVar::Draw()
{
    ...
    DrawTextEx(font, "B", Vector2{static_cast<float>(offsetX - 25 - 5), static_cast<float>(offsetY - cellSize - 10)}, 30, 4, BLACK);
    ...
    for (int i = 0; i < rows; i++)
    {
        for (int j = 0; j < cols; j++)
        {
            ...
            if (j == 0 && i == 0)
            {
                ...
                DrawTextEx(font, "0", Vector2{static_cast<float>(j * cellSize + offsetX + (cellSize - textWIDTH) / 2), static_cast<float>(i * cellSize + offsetY + (cellSize - 25) / 2) - cellSize + 5}, 25, 4, BLACK);
            }
            if (i == 1 && j == 0)
            {
            ...
            {
                int textWIDTH = MeasureText("1", 25);
                DrawTextEx(font, "1", Vector2{static_cast<float>(j * cellSize + offsetX + (cellSize - textWIDTH) / 2), static_cast<float>(i * cellSize + offsetY + (cellSize - 25) / 2) - cellSize + 5}, 25, 8, BLACK);
            }
        }
    }
}